...
//
// Cuando VARIOS tokens comparten el mismo valor resuelto (p.ej. dos colores
// semánticos distintos que hoy son el mismo hex), no hay forma de adivinar
// cuál es "el correcto" solo mirando el valor. Para esos casos, la UI permite
// pegar el JSON exacto de variables de Figma (generado a partir del nombre
// real que tenía cada capa en Figma) y así elegir el token con el mismo
// nombre, en vez de dejarlo sin resolver o -peor- aplicar uno al azar.

// Usamos nombres de archivo versionados (ui.v4.html) en vez de un simple
// "?v=" en la URL: GitHub Pages + el navegador cachean estos archivos con
// bastante agresividad, y en pruebas ni un query param ni un reinstall del
// plugin bastaron para forzar a Penpot a pedir la versión nueva. Un nombre
// de archivo distinto es la única forma 100% segura de que no haya ninguna
// copia vieja cacheada en ningún sitio. Sube el número cada vez que cambies
// este archivo o ui.html (y actualiza también manifest.json → "code").
penpot.ui.open("Reconectar tokens", "ui.v4.html", { width: 400, height: 700 });
...
function buildIndexes(log) {
  ...
  let tokenCount = 0;
  let skippedInactiveSets = 0;

  for (const set of catalog.sets) {
    // Si el set trae explícitamente "active: false", no lo tenemos en cuenta.
    // Si no, lo incluimos (algunas versiones de la API no marcan `active` en
    // todos los sets). Esto evita "ambigüedades" falsas cuando el mismo
    // nombre de token existe repetido en un set inactivo (p.ej. un tema que
    // no está en uso) además del set activo.
    if (set.active === false) {
      skippedInactiveSets += set.tokens.length;
      continue;
    }
  ...
  log(`Catálogo: ${tokenCount} tokens en ${catalog.sets.length} sets${skippedInactiveSets ? ` (${skippedInactiveSets} tokens de sets inactivos ignorados)` : ""}.`);
  return idx;
}
...
// `hints`, si está presente, tiene esta forma (generada a partir de las
// variables REALES de Figma, no adivinada):
//   {
//     color:          { "#eeeff2": ["color/bg/secondary"], ... },
//     fontSizes:      { "14": ["font/subtitle/200/size"], ... },
//     fontWeights:    { "700": ["font/subtitle/200/style"], ... },
//     borderRadius:   { "4": ["radius/md"], ... },
//     borderWidth:    { ... },
//     letterSpacing:  { ... },
//     opacity:        { ... },
//     fontFamilies:   { "forfuture sans": ["font/subtitle/200/family"], ... }
//   }
// `category` indica en cuál de estos mapas hay que mirar, y `matchKey` es la
// clave normalizada (mismo formato que se usó para construir `idx`).
function applyMatch(report, shape, tokenList, props, label, force, hints, category, matchKey) {
  if (!tokenList || tokenList.length === 0) return "unmatched";
  if (!force && props.every((p) => alreadyLinked(shape, p))) return "skipped";

  // Si dos "tokens" candidatos tienen exactamente el mismo nombre (p.ej.
  // porque quedaron definidos en más de un set, o Penpot los devuelve
  // duplicados), no es una ambigüedad real de cara al usuario — aplicar
  // cualquiera de los dos da el mismo resultado visible. Nos quedamos con
  // uno solo por nombre antes de decidir si hay que preguntar.
  let candidates = [];
  ...
  if (
    ...
    matchKey !== null &&
    matchKey !== undefined &&
    hints[category][matchKey] &&
    hints[category][matchKey].length
  ) {
    const wanted = hints[category][matchKey].map(normalizeTokenName);
    const picked = candidates.filter((t) => wanted.includes(normalizeTokenName(t.name)));
    if (picked.length >= 1 && picked.length < candidates.length) {
      candidates = picked;
  ...
  if (candidates.length > 1) {
    ...
    // se pudo desambiguar ni por el valor ni con el JSON de Figma (si lo
    // había, o si el nombre de esa capa concreta no venía en el JSON). No
    // hay forma fiable de adivinar cuál es "el correcto" — así que NO se
    // aplica ninguno automáticamente: se deja constancia para que lo
    // revises tú.
...
function processShape(shape, idx, report, force, hints) {
  ...
  if (Array.isArray(shape.fills)) {
    shape.fills.forEach((fill) => {
      ...
      tally(report, applyMatch(report, shape, idx.color.get(key), ["fill"], "fill", force, hints, "color", key));
    });
  }
    ...
    shape.strokes.forEach((stroke) => {
      if (stroke && stroke.strokeColor) {
        if (!force && stroke.strokeColorRefId) {
          ...
          tally(report, applyMatch(report, shape, idx.color.get(key), ["strokeColor"], "strokeColor", force, hints, "color", key));
        }
      }
      if (stroke && stroke.strokeWidth !== undefined) {
        const key = normalizeNumber(stroke.strokeWidth);
        tally(report, applyMatch(report, shape, idx.borderWidth.get(key), ["strokeWidth"], "strokeWidth", force, hints, "borderWidth", key));
      }
    });
  }
  ...
  if (typeof shape.opacity === "number") {
    ...
    tally(report, applyMatch(report, shape, idx.opacity.get(key), ["opacity"], "opacity", force, hints, "opacity", key));
  }
  ...
  if (corners.every((c) => typeof shape[c] === "number")) {
    ...
    if (allEqual) {
      tally(report, applyMatch(report, shape, idx.borderRadius.get(values[0]), corners, "borderRadius", force, hints, "borderRadius", values[0]));
    } else {
      corners.forEach((c, i) => {
        tally(report, applyMatch(report, shape, idx.borderRadius.get(values[i]), [c], c, force, hints, "borderRadius", values[i]));
      });
    }
  }
  ...
  if (shape.type === "text") {
    ...
    if (typoMatch && typoMatch.length > 0) {
      tally(report, applyMatch(report, shape, typoMatch, ["typography"], "typography", force, hints, null, null));
    } else {
      if (famKey) tally(report, applyMatch(report, shape, idx.fontFamilies.get(famKey), ["fontFamilies"], "fontFamilies", force, hints, "fontFamilies", famKey));
      if (sizeKey !== null) tally(report, applyMatch(report, shape, idx.fontSizes.get(sizeKey), ["fontSize"], "fontSize", force, hints, "fontSizes", sizeKey));
      if (weightKey !== null) tally(report, applyMatch(report, shape, idx.fontWeights.get(weightKey), ["fontWeight"], "fontWeight", force, hints, "fontWeights", weightKey));
      if (lsKey !== null) tally(report, applyMatch(report, shape, idx.letterSpacing.get(lsKey), ["letterSpacing"], "letterSpacing", force, hints, "letterSpacing", lsKey));
    }
  }
}
...
async function run(options, log, done) {
  ...
  if (hints && Object.keys(hints).length > 0) {
    const total = Object.keys(hints).reduce((sum, cat) => sum + Object.keys(hints[cat] || {}).length, 0);
    log(`Usando el JSON de Figma pegado (${total} valores en ${Object.keys(hints).length} categorías) para desambiguar.`);
  }
